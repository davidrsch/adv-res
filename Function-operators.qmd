...
-   Section \@ref(existing-fos) introduces you to two extremely useful existing function operators, and shows you how to use them to solve real problems.

-   Section \@ref(fo-case-study) works through a problem amenable to solution with function operators: downloading many web pages.
...
-   `auto_browse()`: automatically executes `browser()` inside the function when there's an error.
...
2.  Should you memoise `download.file()`? Why or why not?
