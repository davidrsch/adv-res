# Translating R code {#sec-translation}
...
If you're interested in learning more about domain specific languages in general, I highly recommend *Domain Specific Languages* [@dsls]. It discusses many options for creating a DSL and provides many examples of different languages.

### Outline {.unnumbered}

-   Section \@ref(html) creates a DSL for generating HTML, using quasiquotation and purrr to generate a function for each HTML tag, then tidy evaluation to easily access them.

-   Section \@ref(latex) transforms mathematically R code into its LaTeX equivalent using a combination of tidy evaluation and expression walking.
...
-   Special characters start with a `\`: `\pi` = $\pi$, `\pm` = $\pm$, and so on. There are a huge number of symbols available in LaTeX: searching online for `latex math symbols` returns many [lists](http://www.sunilpatel.co.uk/latex-type/latex-math-symbols/). There's even [a service](http://detexify.kirelabs.org/classify.html) that will look up the symbol you sketch in the browser.
