...
-   [Chapter -@sec-names-values], "Nombres y valores ", es un capítulo completamente nuevo que lo ayuda a comprender la diferencia entre objetos y nombres de objetos. Esto lo ayuda a predecir con mayor precisión cuándo R hará una copia de una estructura de datos y sienta las bases importantes para comprender la programación funcional.

-   [Chapter -@sec-vectors-chap], "Vectores" (anteriormente denominadas estructuras de datos), se ha reescrito para centrarse en tipos de vectores como números enteros, factores y marcos de datos. Contiene más detalles de vectores S3 importantes (como fechas y fechas-horas), analiza la variación del marco de datos proporcionado por el paquete tibble [@tibble] y, en general, refleja mi comprensión mejorada de los tipos de datos vectoriales.

-   [Chapter -@sec-subsetting], "Subsetting", ahora distingue entre `[` y `[[` por su intención: `[` extrae muchos valores y `[[` extrae un solo valor (anteriormente se caracterizaban por si "simplificaban" o "conservaban"). La sección @sec-subset-single dibuja el "tren" para ayudarlo a comprender cómo funciona `[[` con listas e introduce nuevas funciones que brindan un comportamiento más consistente para índices fuera de los límites.

-   [Chapter -@sec-control-flow], "Flujo de control", es un nuevo capítulo: de alguna manera me olvidé de herramientas importantes como declaraciones `if` y bucles `for`.

-   [Chapter -@sec-functions], "Funciones", tiene un ordenamiento mejorado, introduce las canalizaciones (`%>%` y `|>`) como una tercera forma de componer funciones (Sección @sec-function-composition) y ha mejorado considerablemente la cobertura de formas de funciones (Sección @sec-function-forms).

-   [Chapter -@sec-environments], "Entornos", tiene un tratamiento reorganizado de entornos especiales @sec-special-environments, y una discusión muy mejorada de la pila de llamadas @sec-call-stack.

-   [Chapter -@sec-conditions], "Condiciones", contiene material previamente en "Excepciones y depuración", y mucho contenido nuevo sobre cómo funciona el sistema de condiciones de R. También le muestra cómo crear sus propias clases de condiciones personalizadas @sec-custom-conditions.
...
-   La programación orientada a objetos (POO) ahora forma una sección importante del libro con capítulos completamente nuevos sobre tipos base ([Chapter -@sec-base-types]), S3 ([Chapter -@sec-s3]), S4 ([Chapter -@sec-s4]), R6 ([Chapter -@sec-r6]) y las compensaciones entre los sistemas ([Chapter -@sec-oo-tradeoffs]).
...
-   El capítulo de memoria ha sido eliminado. Gran parte del material se ha integrado en [Chapter -@sec-names-values] y el resto se sintió demasiado técnico y no tan importante de entender.
